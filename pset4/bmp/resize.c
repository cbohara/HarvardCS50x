...
int main(int argc, char* argv[])
{
    ...
    // create variable to store factor input
    int n = atoi(argv[1]);

    if(n < 1 || n > 100)
    {
        printf("Factor must be greater than 1 or less than 100!\n");
        return 1;
    }
    ...
    // determine dimensions for output image
    bi_r.biWidth  = bi.biWidth * n;
    bi_r.biHeight = bi.biHeight * n;
    
    // determine the old and new paddings
    int padding = (4 - (bi.biWidth * sizeof(RGBTRIPLE)) %4) % 4;
    int paddingResize = (4 - (bi_r.biWidth * sizeof(RGBTRIPLE)) %4) % 4;
    
    // determine output image sizes
    bi_r.biSizeImage = (bi_r.biWidth * sizeof(RGBTRIPLE) + paddingResize) * abs(bi_r.biHeight);
